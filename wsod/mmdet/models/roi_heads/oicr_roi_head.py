...
from ...models.test_time_augs import merge_aug_bboxes
from ...models.layers import multiclass_nms
from ...structures.bbox import bbox2result, bbox2roi, bbox_overlaps
...
class OICRRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def init_bbox_head(self, bbox_roi_extractor: ConfigType, bbox_head: ConfigType):
    ...
    # def loss(self, x, img_metas, proposal_list, gt_labels):
    def loss(self, x: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        bbox_results = self._bbox_forward_train(
            x, proposal_list, gt_labels, img_metas)
        losses.update(
    ...
    def _get_pseudo_gt(self, boxes, cls_prob, gt_labels):
        ...
        for i in range(num_classes):
            if gt_labels[i] == 1:
                ...
                candidate_idx = (cls_prob[:, i] >=
                                 max_prob * 0.9).nonzero().squeeze(1)
    ...
    def _bbox_forward_train(self, x, proposal_list, gt_labels, img_metas):
        ...
        pseudo_gt_1 = self._get_pseudo_gt(
            rois, bbox_results['cls'].clone(), gt_labels)
        pseudo_gt_2 = self._get_pseudo_gt(
            rois, bbox_results['refine1'][:, 1:].clone(), gt_labels)
        pseudo_gt_3 = self._get_pseudo_gt(
            rois, bbox_results['refine2'][:, 1:].clone(), gt_labels)

        refine_labels_1, refine_weights_1, _ = self._sample_rois(
            rois, pseudo_gt_1)
        refine_labels_2, refine_weights_2, _ = self._sample_rois(
            rois, pseudo_gt_2)
        refine_labels_3, refine_weights_3, bbox_targets = self._sample_rois(
            rois, pseudo_gt_3)

        loss_wsddn = self.bbox_head.loss_wsddn(bbox_results['cls'], gt_labels)
        loss_oicr_1 = self.bbox_head.loss_oicr(
            bbox_results['refine1'], refine_labels_1, refine_weights_1)
        loss_oicr_2 = self.bbox_head.loss_oicr(
            bbox_results['refine2'], refine_labels_2, refine_weights_2)
        loss_oicr_3 = self.bbox_head.loss_oicr(
            bbox_results['refine3'], refine_labels_3, refine_weights_3)
        ...
        if bbox_results['bbox'] is not None:
            bbox_targets = self.bbox_head.get_targets(
                rois[:, 1:], bbox_targets[:, 1:])
            loss_bbox = self.bbox_head.loss_bbox(
                bbox_results['bbox'], bbox_targets, refine_labels_3, refine_weights_3)
            bbox_results.update(
                loss_bbox=loss_bbox
            )
        return bbox_results
